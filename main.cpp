...
Node* newNode(char arr[], int g_n, Node* parent) 
{ 
    Node* node = new Node; 
...
vector<Node*> sortCost(vector<Node*> vect)  
{  
    bool swap;  
    Node* temp;  
    ...
    {         
        ...
        {           
            if (vect[count]->g_n > vect[count + 1]->g_n)            
            {               
                temp = vect[count];               
...
Node* makeNodeUniform(char problem[], int size)
{
    ...
    while(!nodes.empty())
    {
        ...
        else
        {
            ...
            // Move tile left
            if(((move_left > -1) && (move_left < size)) && ((b_loc != 0) && (b_loc != 3) && (b_loc != 6)))
            {
                Node* new_node = newNode(node->problem, node->g_n + 1, node);
            ...
            // Move tile right
            if(((move_right > -1) && (move_right < size)) && ((b_loc != 2) && (b_loc != 5) && (b_loc != 8)))
            {
                Node* new_node = newNode(node->problem, node->g_n + 1, node);
                x = new_node->problem[b_loc];
            ...
            // Move tile up
            if(((move_up > -1) && (move_up < size)) && ((b_loc != 0) && (b_loc != 1) && (b_loc != 2)))
            {
                Node* new_node = newNode(node->problem, node->g_n + 1, node);
                x = new_node->problem[b_loc];
            ...
            // Move tile down
            if(((move_down > -1) && (move_down < size)) && ((b_loc != 6) && (b_loc != 7) && (b_loc != 8)))
            {
                Node* new_node = newNode(node->problem, node->g_n + 1, node);
                x = new_node->problem[b_loc];
...
int misplacedTileHeuristic(struct Node* node)
{
    ...
    for(int i = 0; i < SIZE; i++)
        if((node->problem[i] != 'b') && (node->problem[i] != goal_node[i]))
            h_n += 1;
...
int main(int argc, char** argv) 
{
    ...
    switch(x)
    {
        ...
        case 2:
            ...
            temp = makeNodeUniform(problem_arr, SIZE); 

            // Depth 1 state
            int h_n = temp->parent->parent->heuristic - temp->parent->parent->g_n;
            cout << "\nThe best state to expand with a "; 
            cout << "g(n) = " << temp->parent->parent->g_n << " and h(n) = ";
            cout << h_n << " is…\n\t";
            ...
            // Depth 2 state
            h_n = temp->parent->heuristic - temp->parent->g_n;
            cout << "\nThe best state to expand with a "; 
            cout << "g(n) = " << temp->parent->g_n << " and h(n) = ";
            cout << h_n << " is…\n\t";
            ...
            // Goal state
            h_n = temp->heuristic - temp->g_n;
            cout << "\nThe best state to expand with a "; 
            cout << "g(n) = " << temp->g_n << " and h(n) = ";
            cout << h_n << " is…\n\t";
